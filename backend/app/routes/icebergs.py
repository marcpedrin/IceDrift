...
def _load_icebergs_from_ingestion() -> list[dict]:
    """
    Load icebergs from the JSON generated by process_icebergs.py (Task 1).
    Falls back to built-in mock if data not yet ingested.
    """
    import json
    
    # Try reading the JSON file produced by the new data pipeline
    try:
        from config import get_settings
        json_path = Path("data/icebergs.json")
        if json_path.exists():
            ...
            for row in rows:
                result.append({
                    "id":          str(row.get("id", f"UNK{random.randint(10,99)}")),
            ...
            logger.info("Loaded {} icebergs from JSON", len(result))
            if result:
                return result
    except Exception as exc:
        logger.warning("JSON load failed: {}. Trying fallback.", exc)
